...
function validateFootSize(footSize) {
  ...
  if (isNaN(footSize) || parsed < minFootSize || parsed > maxFootSize) {
    throw new Error(
      `Invalid foot size. Must be a number between ${minFootSize} and ${maxFootSize}.`
    );
  }
  return parsed;
}
...
function validateTerrain(terrain) {
  ...
  if (!validTerrains.includes(normalized.toString())) {
    throw new Error(
      `Invalid terrain. Must be one of: ${validTerrains.join(", ")}`
    );
  }
}
...
function validateDistance(distance) {
  ...
  if (isNaN(parsed) || parsed < minDistance) {
    throw new Error(
      `Distance must be a number greater than or equal to ${minDistance} km.`
    );
  }
  return parsed;
}
